...
export default function AdminWebScreen() {
  ...
  async function askAdminAi() {
    const messageText = aiMessage.trim();
    if (!messageText || aiBusy) return;
    setAiBusy(true);
    setAiError("");
    setAiReply("");
    try {
      const { data, error } = await supabase.functions.invoke("admin-ai", { body: { message: messageText } });
      if (error) throw new Error(error.message || "تعذر الاتصال بمساعد الإدارة.");
      const reply = typeof data?.data?.text === "string" ? data.data.text.trim() : "";
      if (!reply) throw new Error("عاد المساعد دون نص قابل للعرض.");
      setAiReply(reply);
    } catch (error: unknown) {
      setAiError(error instanceof Error ? error.message : "تعذر الاتصال بمساعد الإدارة.");
    } finally {
      setAiBusy(false);
    }
  }
  ...
  function editCourse(course: SupabaseCourse) { setEditingCourseId(course.id); setSelectedCourseId(course.id); setCourseTitle(course.title); setCourseDescription(course.description ?? ""); setCourseInstructor(course.instructor ?? ""); setCourseCategory(course.category_id); setCoursePublished(course.is_published); setMessage(""); }
  function resetCourseForm() { setEditingCourseId(null); setCourseTitle(""); setCourseDescription(""); setCourseInstructor(""); setCourseCategory(null); setCoursePublished(true); }
  async function saveCourse() { if (!courseTitle.trim()) return; setBusy(true); const payload = { title: courseTitle.trim(), slug: `${courseTitle.trim().toLowerCase().replace(/[^a-z0-9]+/g, "-")}-${Date.now()}`, description: courseDescription.trim() || null, instructor: courseInstructor.trim() || null, category_id: courseCategory, is_published: coursePublished }; const result = editingCourseId ? await supabase.from("courses").update(payload).eq("id", editingCourseId).select().single() : await supabase.from("courses").insert(payload).select().single(); if (result.error) setAuthError(result.error.message); else { setCourses((current) => editingCourseId ? current.map((course) => course.id === editingCourseId ? result.data as SupabaseCourse : course) : [result.data as SupabaseCourse, ...current]); setSelectedCourseId((result.data as SupabaseCourse).id); resetCourseForm(); setMessage("تم حفظ الكورس."); } setBusy(false); }
